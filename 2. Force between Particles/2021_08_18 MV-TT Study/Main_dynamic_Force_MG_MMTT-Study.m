...
addpath('C:\Users\Matt Gorman\OneDrive\Documents\MATLAB\InducedCharge_git\Spherical Particles\ParticleFiles_mat');
addpath('C:\Users\Matt Gorman\OneDrive\Documents\MATLAB\InducedCharge_git\Spherical Particles');
addpath('C:\Users\Matt Gorman\OneDrive\Documents\MATLAB\InducedCharge_git\2. Force between Particles\Particle Data');
addpath('C:\Users\Matt Gorman\OneDrive\Documents\MATLAB\InducedCharge_git\2. Force between Particles\2021_08_18 MV-TT Study');
...
filename = 'C:\Users\Matt Gorman\OneDrive\Documents\MATLAB\InducedCharge_git\2. Force between Particles\2021_08_18 MV-TT Study\ParData_B0p2_beta6_rotateZ_NegPiDiv12_21924.mat'; % Geometry: Axisymmetric ellipsoid
filename2 = 'C:\Users\Matt Gorman\OneDrive\Documents\MATLAB\InducedCharge_git\2. Force between Particles\2021_08_18 MV-TT StudyParData_B0p2_beta6_rotateZ_piDiv12_21924.mat';
filename3 = 'C:\Users\Matt Gorman\OneDrive\Documents\MATLAB\InducedCharge_git\2. Force between Particles\Particle Data\ParData_beta6_B0p2_2884.mat';
npars = 2;%2;%4;
...
sigma_save_mg = zeros(2884*npars,length(gap));
x_save_mg = zeros(2884*npars,length(gap));
y_save_mg = zeros(2884*npars,length(gap));
z_save_mg = zeros(2884*npars,length(gap));
...
% Read patch info
% [x_rel,y_rel,z_rel,DeltaArea,NN,NormVec,a,b,c]=F_GeometryGluedSphere(filename3);
[x_rel,y_rel,z_rel,DeltaArea,NN,NormVec,a,b,c]=F_GeometryAxiSphere(filename);
[x_rel2,y_rel2,z_rel2,DeltaArea2,NN2,NormVec2,a,b,c]=F_GeometryAxiSphere(filename2);
x_all = [x_rel + xpar(1); x_rel2 + xpar(2)];
y_all = [y_rel + ypar(1); y_rel2 + ypar(2)];
z_all = [z_rel + zpar(1); z_rel2 + zpar(2)];
...
%     fprintf('==========================\n');
    ...
    sigma_save_mg(:,ll) = sigma;
    x_save_mg(:,:,ll) = reshape(x_pat,[npars*2884,1]);
    y_save_mg(:,:,ll) = reshape(y_pat,[npars*2884,1]);
    z_save_mg(:,:,ll) = reshape(z_pat,[npars*2884,1]);
    ...
    F_base = pi*(abs(sigma_f_scalar(1)))*(abs(sigma_f_scalar(2)))/1;% pi*(1^2)*(1^2)/1;
...
%     figure(99);
    ...
    scatter3(xplot,yplot,zplot,patchSize,sigma,'filled');
    cbar = colorbar;
...
figure;
scatter(gap,Fdim_save_mg,10,'r','filled');
box on; axis equal; grid on;

filenameStr = ['ParData_Force_temp_',num2str(length(x_rel))];
uinp = input(['Would you like to save this force data as ', filenameStr ,'.mat (y/n)?\n   '],'s');
if(strcmp(uinp,'y') == 1)
    save(filenameStr,'gap','Fdim_save_mg','U_save_mg','sigma_save_mg','x_save_mg', ...
        'y_save_mg','z_save_mg');
end
