function [sigma_b2,b2] = F_getSigmaB_Arb_Matrix(numPars,NpatchesPar,dAmat,x,y,z,nVect,x_pcs,y_pcs,z_pcs,pcharge,sigma_f,k_air,k_obj,Ext_EField_x,Ext_EField_y,Ext_EField_z)
...
for n = 1:numPars
    if i == 1
        ppld(1:NpatchesPar(1),1:NpatchesPar(1),1) = 0;
        ppld(1:NpatchesPar(1),1:NpatchesPar(1),2) = 0;
        ppld(1:NpatchesPar(1),1:NpatchesPar(1),3) = 0;
    else
        index = sum(NpatchesPar(1:n-1));
        ppld(index + 1 : index+NpatchesPar(n),index + 1 : index+NpatchesPar(n),1) = 0;
        ppld(index + 1 : index+NpatchesPar(n),index + 1 : index+NpatchesPar(n),2) = 0;
        ppld(index + 1 : index+NpatchesPar(n),index + 1 : index+NpatchesPar(n),3) = 0;
    end
end
