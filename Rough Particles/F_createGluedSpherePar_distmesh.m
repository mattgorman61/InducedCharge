function [x,y,z,dA,V,nVect] = F_createGluedSpherePar_distmesh(R,dxs,dys,dzs)
% PROVIDES ...
%{  
...
[p,t]=distmeshsurface_gluedPars(@fd_gluedSpheres,@huniform,elemSize,5*[-R_0,-R_0,-R_0; R_0,R_0,R_0]);
...
for i = 1:NpatchesSph
    ...
    r_candidates = sqrt((x_i-dxs).^2 + (y_i-dys).^2 + (z_i-dzs).^2 );

    A = abs(r_candidates - r_spheres');
    minRadDiff = min(min(A));
    [rCandInd,sphInd] = find(A==minRadDiff);
    minRadDiffCheck = A(rCandInd,sphInd);
    
%     r_i = r_candidates(rCandInd);
    sphereXLoc = dxs(sphInd);
    sphereYLoc = dys(sphInd);
    sphereZLoc = dzs(sphInd);
