function [F1,F2,M1,M2] = F_CollMomTrans(mm,nn,a,b,c,Elastic,Poisson,RMatrx,...
    Ematrx_inc,rpar,vpar,rot_pf,frict,Contact1,Contact2,time,fid,erest,mass,thickness)
...
%collision. Soft sphere model is used here.
    ...
    Q1 = reshape(Ematrx_inc(mm,:,:),[4,4]);
    Q2 = reshape(Ematrx_inc(nn,:,:),[4,4]);
    ...
    rot1 = inv(Rot1)* reshape(rot_pf(mm,:),[3,1]); 
    rot2 = inv(Rot2)* reshape(rot_pf(nn,:),[3,1]);
...
%     fprintf('Damping coefficient = %f\n',damp);
    ...
    fprintf (fid,'%10.4f %10.4f %10.4f %10.4f %10.4f %10.4f %10.4f %10.4f %10.4f %10.4f\n',...
        time,delta_n,dot(vrel,nVec1),norm(vrel_tangent),...
        F1(1),F1(2),F1(3),M1(1),M1(2),M1(3));
