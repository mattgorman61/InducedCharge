...
%   此处显示详细说明
    ...
    if(sum(abs(lamda_Img)>(1E-10))>0) 
    % If the image of eigen value is very small, it is real
    % Setting criterion to 0 could cause problem due to numerical errors! 
        lcollide = 1;
%       fprintf('Collision detected: i=%d, j=%d!\n',i,j);
        
        % Find out collision point here
        [Contact1]=F_CollPoint(1,2,Ematrx_inc);
        [Contact2]=F_CollPoint(2,1,Ematrx_inc);
